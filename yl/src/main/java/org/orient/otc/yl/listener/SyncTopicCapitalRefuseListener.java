...
@RocketMQMessageListener(topic = RocketMqConstant.SYNC_TOPIC
        ...
        , selectorExpression = RocketMqConstant.CAPITAL_REFUSE
        , consumerGroup = RocketMqConstant.CAPITAL_REFUSE
        , consumeMode = ConsumeMode.CONCURRENTLY)
@Component
@Slf4j
public class SyncTopicCapitalRefuseListener implements RocketMQListener<CapitalRecordsVO>, RocketMQPushConsumerLifecycleListener {
    @Resource
    ...
    public void onMessage(CapitalRecordsVO capitalRecordsVO) {
        ...
        //推送确认记录
        ylService.clientCashInCashOutRefuse(capitalRecordsVO.getYlId());
    }

    @Override
    public void prepareStart(DefaultMQPushConsumer consumer) {
        consumer.setInstanceName(RocketMqConstant.SYNC_TOPIC + "-" + RocketMqConstant.CAPITAL_REFUSE);
    }
}
