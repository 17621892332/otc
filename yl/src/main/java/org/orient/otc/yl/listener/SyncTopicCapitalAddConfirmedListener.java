...
@RocketMQMessageListener(topic = RocketMqConstant.SYNC_TOPIC
        ...
        , selectorExpression = RocketMqConstant.CAPITAL_ADD_CONFIRMED
        , consumerGroup = RocketMqConstant.CAPITAL_ADD_CONFIRMED
        , consumeMode = ConsumeMode.CONCURRENTLY)
@Component
@Slf4j
public class SyncTopicCapitalAddConfirmedListener implements RocketMQListener<CapitalRecordsVO>, RocketMQPushConsumerLifecycleListener {
    @Resource
    ...
    public void onMessage(CapitalRecordsVO capitalRecordsVO) {
        ...
        //推送确认记录
        ylService.clientCashInCashOutConfirmed(clientCashInoCashOutVO.getId());
    }

    @Override
    public void prepareStart(DefaultMQPushConsumer consumer) {
        consumer.setInstanceName(RocketMqConstant.SYNC_TOPIC + "-" + RocketMqConstant.CAPITAL_ADD_CONFIRMED);
    }
}
