...
public class SwaggerConfiguration {
	...
	public Docket baseRestApi() {
		return new Docket(DocumentationType.SWAGGER_2).apiInfo(apiInfo()).select()
				//.apis(RequestHandlerSelectors.basePackage("org.orient.otc.netty.controller")).paths(PathSelectors.any())
				.build()
	...
	public ApiInfo apiInfo() {
		return new ApiInfoBuilder().title("netty模块").description("netty模块接口文档").termsOfServiceUrl("").version("1.0").build();
	}
